...
interface SessionPickItem extends vscode.QuickPickItem {
  sessionId: string;
  targetKind: ScriptTargetType;
}

type ScriptTargetSession = ActiveSession | ActiveSerialSession | ActiveLocalShellSession;
...
export async function pickTarget(
  ...
  const localCandidates: Array<{
    session: ActiveLocalShellSession;
    profileId: string;
    profileName: string;
    label: string;
    description: string;
  }> = wantLocal
    ? (snapshot.activeLocalShellSessions ?? []).map((s) => {
        const profileName = (snapshot.localShellProfiles ?? []).find((p) => p.id === s.profileId)?.name ?? s.profileId;
        return {
          session: s,
          profileId: s.profileId,
          profileName,
          label: s.terminalName,
          description: `Local Shell • ${profileName}`
        };
      })
    : [];
  ...
  if (descriptor.targetProfile) {
    ...
    const localById = localCandidates.find((c) => c.profileId === descriptor.targetProfile);
    if (localById) return localById.session;
    ...
    if (nameMatches === 1) {
      return (sshByName[0]?.session ?? serialByName[0]?.session ?? localByName[0]?.session) as ScriptTargetSession;
    }
    if (nameMatches > 1) {
      const narrowed: Array<SessionPickItem & { session: ScriptTargetSession }> = [
        ...sshByName.map((c) => ({
          label: c.label,
      ...
      })) as (SessionPickItem & { session: ScriptTargetSession }) | undefined;
