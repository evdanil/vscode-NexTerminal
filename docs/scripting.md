...
3. **Create a script** at `.nexus/scripts/hello.js` (or simply run `Nexus: New Nexus Script` — it writes to the resolved scripts directory wherever it lives):
   ...
   ```
4. **Run it** — any of these ways. The first three bind to a session that is already open; the last opens a new one. The script's opening works for both:
...
The first time you run a script in this workspace (from any entry point — even if you then cancel the session picker), Nexus writes `types/nexus-scripts.d.ts` + `jsconfig.json` next to your scripts so the editor gives autocomplete, JSDoc hovers, and inline type-checking for every primitive. The `jsconfig.json` is then yours to edit — Nexus never rewrites one you've changed, and writes the bundled copy again if you delete it. (The one exception is an unedited copy written by Nexus 2.8.0–2.8.2, which is replaced with the current one.)
...
- The run ends when the body finishes or throws, and whatever the script left running ends with it: a call it didn't `await` or a `setTimeout` still pending never sends to the terminal or opens a dialog afterwards.
...
A dialog belongs to its run. If the run ends while one is open — you stop it, its session drops, or the script finishes without awaiting it — Nexus closes a `prompt` box. VS Code gives an extension no way to close a modal dialog, so a `confirm` or `alert` stays until you answer it; that answer then goes nowhere, and a notification says it was ignored. Nothing else a script leaves behind outlives its run either (see [Anatomy of a script](#anatomy-of-a-script)).
...
| `"InvalidPath"` | any `nexus.fs.*` call | The path is empty, not a string, contains a NUL byte, or (Windows) is drive-relative (`"C:file.txt"`) — or it contains a `\` while the script is saved under a URI other than `file:` (a `vscode-remote:` script, which is what Nexus sees when `remote.extensionKind` makes it run on your local machine in a remote window, or a script on a file system another extension provides); use `/` there. In an ordinary Remote-SSH, WSL or Codespaces window Nexus runs on the remote host, the script is a `file:` path there, and `\` follows that host's rules. |
| `"PathOutsideScope"` | any `nexus.fs.*` call | The resolved path lands outside both allowed roots. |
| `"FileNotFound"` | `readText` | Nothing exists at the path, or it's a directory (or, when the script is a `file:` path, not a regular file — a device or a pipe). |
| `"FileTooLarge"` | `readText` | The file is bigger than the run's effective cap (`nexus.scripts.maxReadSizeMb`, default 4 MiB). `err.sizeBytes` / `err.maxBytes` carry the numbers — `sizeBytes` is normally the size the file system reports, which is checked before anything is read. If the file turns out bigger than that (it grew past the cap mid-read, or its size was under-reported), `sizeBytes` is a lower bound, the cap + 1, when the script is a `file:` path (on your machine, or on the remote host in an ordinary Remote-SSH, WSL or Codespaces window; see the `InvalidPath` row), and the number of bytes actually received when it is saved under any other URI. |
...
**Module identity is the spelled path, not the inode.** On Windows, `./Lib.js` and `./lib.js` are the same module; everywhere else they are two — including on a **case-insensitive macOS volume** (APFS's default), where the disk would serve one file for both spellings but the include cache keeps them distinct, so the file's body runs twice and each spelling gets its own exports object. The same happens for a script on a **Windows host reached through a non-`file:` URI** — a `vscode-remote:` script when `remote.extensionKind` makes Nexus run on your local machine in a remote window, or a file system another extension provides — because such paths are handled as POSIX (a URI carries no way to know the far host's OS). An ordinary Remote-SSH window to a Windows host is not one of these: Nexus runs on that host, the script is a `file:` path there, and the Windows rule applies. Two **symlinked paths** to one file are two modules as well. Identity is deliberately lexical — no `stat` or `realpath` inside the cache and cycle checks, which also means no way to probe the volume's case behaviour — so the rule to remember is simply: **include each file by one spelling**. Double-loading is bounded like everything else (both spellings count against the module and byte budgets) and cannot affect containment.
...
- Each running script owns a rolling buffer of the session's recent output (the last 65,536 characters; ANSI escapes stripped at write time so patterns match on the same characters the user sees). **The buffer starts when the run starts**: anything the session printed before that — including a prompt already sitting on screen — is never in it. Under **Connect and Run Script…** on an SSH or Telnet server or a serial profile the run starts with the session itself: the buffer holds the session's output from the moment it opens while Nexus reads the script file. On serial, data received before the port-open response is bounded to 64 KiB or 256 chunks; overflow drops the oldest chunks, and a single chunk larger than 64 KiB is discarded.
- The buffer has a **forward-only cursor**. A wait scans from the cursor to the end of the buffer, then keeps scanning as new output arrives. Until something has matched, that window is everything received since the run started — under **Connect and Run Script…** on a server or a serial profile, since the session opened (subject to the serial pre-response bound above), so the first match's `before` can hold the host's banner or the device's boot messages; after that, only output following the previous match.
...
- Per-call `lookback: n` widens the window to include `n` characters before the cursor — use it (e.g. `lookback: 4096`) to re-match a banner or prompt an earlier wait already consumed. It never reaches output from before the run started (under **Connect and Run Script…** on a server or a serial profile, before the session opened; serial output dropped at the pre-response bound is not retained either).
...
- **A wait matches a spare prompt**, because the script pressed Enter although the host's first prompt was already in its buffer or on its way. That happens under **Connect and Run Script…** on an SSH or Telnet server: the run keeps the session's output from the moment it registers — when the SSH shell channel opens, or the Telnet TCP connection is up — so the host's first prompt is in the buffer or on its way, and an opening `sendLine("")` makes a second one that the wait after your first command matches before the command has answered. So wait briefly for a prompt and press Enter only if none came, as the Quickstart does: there the wait finds the first prompt, and on an already-open terminal it times out and the Enter follows. **Connect and Run Script…** on a serial profile keeps the session's output from when the port starts opening too; while its open response is pending, the retained data is bounded to 64 KiB or 256 chunks, dropping oldest chunks on overflow (and discarding a single chunk larger than 64 KiB). A port usually prints nothing as it opens, so there the wait times out and the Enter follows — but early boot messages are in the buffer unless that bound is exceeded. **Open and Run Script…** on a Local Shell profile starts the run once the shell has been running for about five seconds, with its first prompt already on screen, as in any open terminal.
...
| `Connect/Open and Run Script…` (server, serial, or Local Shell right-click) | — | Pick a Nexus script, open the profile, and run the script against the new session the moment it registers — for SSH when the shell channel opens, for Telnet when the TCP connection is up, for Serial when the port starts opening. On a server the run keeps the session's output from that moment; on Serial it queues up to 64 KiB or 256 data chunks until the port-open response, dropping oldest chunks on overflow and discarding a single chunk larger than 64 KiB (see [Match window semantics](#match-window-semantics)). Scripts are filtered to those whose `@target-type` is compatible with the profile. SSH, Telnet and Serial use a 90-second watchdog; Local Shell starts the run once the shell has been running for about five seconds. |
