...
| `"FileTooLarge"` | `readText` | The file is bigger than the run's effective cap (`nexus.scripts.maxReadSizeMb`, default 4 MiB). `err.sizeBytes` / `err.maxBytes` carry the numbers — `sizeBytes` is the size actually found, except when the script is a `file:` path (on your machine, or on the remote host in an ordinary Remote-SSH, WSL or Codespaces window; see the `InvalidPath` row) and the file's size couldn't be trusted (it grew past the cap mid-read, or its reported size was wrong): then it is a lower bound, the cap + 1. |
