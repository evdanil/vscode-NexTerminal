...
| `nexus.scripts.maxReadSizeMb` | number (MiB) | `4` | Largest file `nexus.fs.readText` / `nexus.fs.readJson` will read; bigger files throw `FileTooLarge`. Range 1–16 MiB; out-of-range or non-numeric values fall back to 4 MiB. Snapshotted when a run starts, so a change never applies to a script already running. |
