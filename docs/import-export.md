...
Share files include only explicitly allowlisted portable preferences. Host-trust and logging/privacy choices, script and macro automation controls, every directory path, bind addresses and interfaces, all Network Servers and Local Servers settings stay on the sender. Import applies the same allowlist again, so an older or hand-edited share cannot overwrite those settings on the recipient. Encrypted backup and restore keep their existing settings behavior.
