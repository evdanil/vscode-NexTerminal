...
**Replace keeps a saved password only for an unchanged server.** Replace removes your servers before it imports the file's. The password, key passphrase and proxy password this machine saved for a server stay only if the file brings that server back with the same address: host, alternate host, port, username and proxy all unchanged — its name, folder and other settings can differ. Otherwise they are deleted, and you're asked for them on the next connect; the import's summary says how many servers that affected. This covers files the seal cannot check — a backup from before 2.8.243, a hand-written file, a backup whose encrypted part was removed — so none of them can send a password you saved to a host it was never meant for. Passwords the backup itself carries are still restored, and Merge leaves your servers and their passwords alone.
