...
Share files include only explicitly allowlisted portable preferences. Host-trust and connection-isolation choices, logging capture/retention/privacy settings, SFTP background-watch behavior and resource/safety limits, script and macro automation/runtime/read/context controls, every directory path, bind addresses and interfaces, all Network Servers and Local Servers settings stay on the sender. Import applies the same allowlist again, so an older or hand-edited share cannot overwrite those settings on the recipient. Encrypted backup and restore keep their existing settings behavior.
...
- **Share Export**: Run `Nexus: Export for Sharing` to create a sanitized export safe for sharing (credentials stripped, Local Shell environment variables removed, learned hardware identifiers removed, IDs remapped). Your inventory sources go with it, together with the device templates they use, your saved filters and the servers the sources synced — addressless ones included — so the servers arrive already belonging to their source, and the recipient's first sync updates them where they are instead of adding them a second time. A source never carries its saved token or password, a login typed into its URL, or when it last synced. A NetBox, EVE-NG, Proxmox or GNS3 source also arrives with **Allow a Self-Signed or Mismatched Certificate** turned off, an EVE-NG, Proxmox or GNS3 source without its status poll interval, and an EVE-NG or GNS3 source without your provider login. The recipient enters their own credentials in **Edit Inventory Source** (the import's notification has a button for it) before syncing; for a source that uses none, the notification asks only for a review. A key auth profile travels without its key file, so it arrives unlinked from the sources, device templates and synced servers that used it: the recipient gives it their own key file, or picks another profile, and links it on the source. Profile-scoped macro triggers are relinked to server, serial, or Local Shell profiles in the bundle; if the profile is not included or does not import, all trigger settings are removed from the macro. A source set to delete servers whose device disappears arrives set to move them to `_orphaned` instead — see [Sources from a Shared Export](inventory/README.md#sources-from-a-shared-export). Someone on a version before 2.8.253 gets the servers but not their sources, so ask them to update first. Importing a share applies the same allowlist again, so a hand-edited or older file cannot restore excluded security and privacy choices, directory paths, bind addresses or interfaces, or local Network Servers or Local Servers settings. Each shared record is also rebuilt from a typed per-model allowlist on export and import: fields are kept, dropped, rewritten or relinked explicitly, so unknown or hand-added fields do not travel and model additions require a declared sharing rule before compilation succeeds.

An addressless inventory server restored from a backup or share keeps its settings. If the file carries old console host or port ownership stamps alongside that addressless state, import discards those stamps so the server can take a newly reported address in place on its next sync.
...
**Replace keeps a saved password only for an unchanged server.** Replace removes your servers before it imports the file's. The password, key passphrase and proxy password this machine saved for a server stay only if the file brings that server back with the same address: host, alternate host, port, username and proxy all unchanged — and, for a server reached through an SSH jump host, the same goes for the jump host, and for any jump host behind that. Its name, folder and other settings can differ. Otherwise they are deleted, and the import's summary says how many servers that affected. If the import stops partway with an error, the servers it had already removed lose their saved passwords too, rather than leaving them for whatever comes back under the same id. This covers files the seal cannot check — a backup from before 2.8.243, a hand-written file, a backup whose encrypted part was removed — so none of them can send a password you saved to a host it was never meant for. Passwords the backup itself carries are still restored, and Merge leaves your servers and their passwords alone.

Replace closes open sessions and stops running tunnels for profiles the backup does not restore under the same id. Serial and Local Shell terminals for removed profiles close too. A profile restored with the same id keeps its open runtime. Cleanup runs after the config write; if a connection cannot be stopped, Nexus offers to reload the window.
