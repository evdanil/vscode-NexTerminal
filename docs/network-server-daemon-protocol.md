...
* A daemon receiving a host line that is not valid JSON MAY ignore it with a
  warning when no usable request id can be answered. The host is stricter in
  the daemon-to-host direction: malformed daemon stdout is a protocol failure
  and the current child is retired.
* Readers MUST bound the length of a single line. Both implementations use
  **1 MiB** (see §7.3). This is not theoretical politeness: an
...
2. **Event** — has a string `event` **and** an own `data` key (of any type,
   including `null`).
3. Anything else from daemon stdout is a protocol failure; the host MUST retire
   the current child instead of continuing with an untrusted stream.
...
```jsonc
{ "id": 7, "error": { "code": "INVALID_REQUEST", "message": "Malformed RPC request." } }
```
...
| Code | Meaning |
| --- | --- |
| `INVALID_REQUEST` | Unknown method, unknown service id, malformed params, or configuration DTO rejected before an operation is applied. |
...
**Errors:** `INVALID_REQUEST` when `id` names no known service.
...
**Errors:** `INVALID_REQUEST` for an unknown id or malformed config; `INTERNAL_ERROR` when the service
...
**Errors:** `INVALID_REQUEST` when `id` is any service other than `"tftp"` — only TFTP
...
**Errors:** `INVALID_REQUEST` for an unknown id or a service with no runtime view.
...
count in the Rust daemon. Additional counter fields MUST NOT be emitted on the
RPC boundary unless the host schema is updated first; the current host validates
the exact DTO shape.
...
The daemon MUST bound the length of a stdin line at **1 MiB** and MUST discard —
