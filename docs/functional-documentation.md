...
- Vault write ordering is a crash contract: the secret-id ledger names an id **before** the value is stored under it, `nexus.macros` is written next, and vault deletes plus the ledger shrink happen **last**. A crash at any point therefore leaves an entry that `Complete Reset` can still find, and never deletes the only durable copy of a value that is being moved to a new key.
