...
- Vault write ordering is a crash contract: every secret id is NAMED before the value is stored under it, `nexus.macros` is written next, and vault deletes plus the shrink happen **last**. A crash at any point therefore leaves an entry `Complete Reset` can still find, and never deletes the only durable copy of a value being moved to a new key. The naming is now a MARKER FILE per secret id under `context.globalStorageUri` (`ensureSecretMarkers()`), written with `vscode.workspace.fs` and enumerated by `clearAll()` with `readDirectory()`. That medium was chosen because it is the only one whose writes are genuinely per-entity: the previous `nexus.macros.secretIds` array is a read-modify-write on `globalState`, so two windows adding a secret at the same moment each wrote their own union and one id ended up named by nothing — an unsweepable plaintext secret in the OS keyring, reproduced in review. One `globalState` KEY per id does not help either, and `updateSecretIndex()` records why: `ExtensionMemento.update()` persists the entire memento as a single storage record, so distinct keys clobber one another exactly as a shared array does. The array is still written and still read by `Complete Reset`, as a legacy fallback for whatever it still names from builds that predate the marker files. What NEITHER record rescues is an entry named by nothing, and there are two such cases: a pre-marker entry whose own ledger write lost that race, and — because a marker bounds what a SAVE must do, not what another WINDOW may delete afterwards — an entry re-created by a stale save whose marker a concurrent delete removed while that save was blocked inside `secrets.store()` (the interleaving is spelled out in `clearAllExclusive()`; no ordering of unconditional writes at the start of a save can prevent it, and re-marking after the store only moves the window). `clearAll()` therefore also enumerates the vault itself, via `SecretStorage.keys()`, filtered to the `macro-secret-text-` prefix so that server passwords, passphrases and auth-profile secrets — which share the namespace — are never touched. That API was finalized in VS Code 1.105 while `engines.vscode` is `^1.95.0`, so it is FEATURE-DETECTED (`typeof this.context.secrets.keys === "function"`) and every name-based source is kept for older hosts, where those two leftovers do survive a reset until the host is updated. Raising the engine floor would make the enumeration unconditional and is a compatibility decision, not a bug fix. The marker write FAILS CLOSED: it is the first write of a save, and if it cannot be written the save is abandoned with an error naming the folder, before any vault entry, ledger entry or `nexus.macros` write — Nexus does not store a secret it could not name first. Cost: one `createDirectory()` plus one `writeFile()` per SECRET id in the save, unconditionally and on every save. The write is deliberately not memoized per window — a marker is shared state that another window legitimately deletes (on a macro delete, or on `Complete Reset`), and because a save republishes every secret the window holds, a "this window already wrote it" skip could put a live vault entry back with no marker naming it. The bytes are a pure function of the id, so a repeat write is idempotent and two windows racing write identical files. A save with no secret macros writes no markers, but the folder is still opened by `removeSecretMarkers()` when a formerly-secret id's vault entry is deleted (a macro removed, or turned plain); that call is best-effort rather than fail-closed, so a dead share cannot undo the deletes it describes. Which ids that removal is attempted for is narrowed to "was secret in the resolved list a moment ago" UNION "this instance has actually written a marker for" (`markedSecretIds`) — the second half covers an id whose marker and vault entry landed in a save that then failed at its `nexus.macros` commit, so the in-memory list never advanced and a retry saving the same macro as plain left the marker on disk indefinitely. That set is consulted ONLY to widen a removal, never to skip a marker WRITE, which is the memoization that was removed in review and must not come back.
