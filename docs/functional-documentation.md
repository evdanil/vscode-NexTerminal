...
- **Removed in 2.8.74, deliberately, and it should not come back.** Nexus used to maintain its own record of "vault keys this extension may have written", so that `clearAll()` had a list of keys to try: first the `nexus.macros.secretIds` array in `globalState`, then one marker FILE per secret id under `context.globalStorageUri` alongside it. Both imposed an ordering contract on every write path (grow the record before `secrets.store()`, shrink it after the delete) and the marker write was fail-closed, so an unwritable storage folder failed a save. Both were also cross-window read-modify-writes over shared state and could therefore lose an id, and three successive review rounds each found a fresh way for one to strand or leak an entry. None of that is fixable by ordering, because the write that UNNAMES an entry happens after the writes that name it. Enumeration is not a record of what was written but a question asked of the thing that holds it, so it has no such race — which is why the whole class was deleted rather than repaired. Nothing reads or writes either record now. A profile upgrading from an earlier build keeps whatever `nexus.macros.secretIds` it had; it is inert, so a stale entry can cause neither a wrong deletion nor a missed sweep, and no first-run cleanup was added for it (a one-shot migration to erase bytes nothing reads would be more mechanism than the bytes cost). The marker files never shipped in a release.
