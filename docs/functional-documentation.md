...
- **`group` is untrusted at every read site** (`sanitizeMacroGroup()` / `macroGroup()` in `src/services/macroFolders.ts`): a non-string, `".."`, an over-depth path, or a pathologically long string all canonicalize to `undefined` rather than throwing or stalling the extension host. `""` and a missing `group` are indistinguishable — both render at the root. This guard runs at every read site, plus both ingest paths that can carry attacker- or hand-edit-controlled data into `MACROS_KEY` — `persistLegacyMigration` (settings absorption, replayed on every activation including Settings Sync) and `reloadFromState` (a malformed value already sitting in storage) — and in `sanitizeImportedMacro` for the config-import path. The persisted folder list gets the same treatment: non-strings are dropped, everything else is normalized.
