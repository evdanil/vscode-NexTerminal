...
- **Scan bounds** (`SCRIPT_SCAN_MAX_DEPTH = 10`, `SCRIPT_SCAN_MAX_ENTRIES = 500` in `scriptScanner.ts`): recursion stops past 10 levels of nesting (a folder discovered exactly at the cap still renders, just unexpanded); the entry budget counts every directory and every non-`.js` file examined — not `.js` files themselves, so a directory full of scripts is never artificially truncated — and once exceeded, the scan stops and a truncation node is pinned first at the root of the tree (its click action opens the `nexus.scripts.path` setting). Dot-directories and `node_modules` are skipped case-insensitively (relevant on WSL2's case-insensitive `/mnt/c`); `.js` matching is likewise case-insensitive. The generated `<scriptsDir>/types/` directory is skipped only at the scripts root — a user's own `cisco/types/probe.js` is a normal, scanned folder. Directory-type detection uses a bitmask test (`type & vscode.FileType.Directory`, not strict equality), so a symlinked directory (an NTFS junction surfaced through WSL2, for example) is followed as a folder rather than silently dropped; the depth and entry caps are what bound the cost of a symlink cycle, not a separate symlink check.
