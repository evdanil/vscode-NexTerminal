...
- Built-in templates are **Send command**, **Send password when prompted**, **Wait and send confirmation**, **Scoped auto-trigger example**, **IPMI SOL console**, and **Launch IPMI web console** (see 4.9.2).
...
- The **IPMI SOL console** template (4.9.1) builds `ipmitool -I lanplus -H ${profile.ipmiHost} -U $username -P $password sol activate` with `username` (label "Username") and `password` (label "Password", masked) declared and `runIn: "localTerminal"` — the originating feature request's worked example end to end, with the BMC address read from the server profile (issue #48) instead of prompted for.
