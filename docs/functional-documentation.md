...
- Vault write ordering is a crash contract: the secret-id ledger names an id **before** the value is stored under it, `nexus.macros` is written next, and vault deletes plus the ledger shrink happen **last**. Within one window a crash at any point therefore leaves an entry `Complete Reset` can still find, and never deletes the only durable copy of a value being moved to a new key. It is NOT a cross-window guarantee: the ledger is a read-modify-write on `globalState`, so two windows adding a secret at the same time can each write their own union and lose the other's — and because both saves write the ledger and `nexus.macros` in the same order, the same window wins both, so one lost race is enough to leave a vault entry named by neither key. `updateSecretIndex()` documents this, including why the obvious fix (one `globalState` key per secret id) does not work: `ExtensionMemento.update()` persists the entire memento as a single storage record, so distinct keys clobber one another exactly as a shared array does. A per-id marker file under `globalStorageUri` would fix it and is deferred.
...
- The cross-window "don't rewrite a secret this window did not change" rule is a DEFERRAL, not a skip: `save()` collects those stores and, before committing `nexus.macros`, reads each key back. A key holding a different value is still left alone (that is the whole point — a stale window must not revert a newer password). A key that has GONE is rewritten with the value this window last observed, because the `nexus.macros` write is about to republish that record as `secret: true` and a secret macro on disk with no vault entry behind it is an empty secret at the next load, reported to nobody. Both routes to it are ordinary: another window running `Complete Reset`, or another window simply deleting that macro.
