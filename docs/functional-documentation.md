...
- Drag and drop: a macro tree row drags with its own MIME type (`application/vnd.nexus.macro`, distinct from the Connectivity Hub's tunnel/profile MIME so the two views don't advertise cross-acceptance) and payload `macro.id` (never an index, so a concurrent array change can't rewrite the wrong macro's folder). Dropping onto a folder sets that macro's `group`; dropping onto another macro targets that macro's own folder; dropping onto the root clears `group`. Folders themselves are not draggable. The tree has no multi-select — bulk moves go through `moveToFolder`'s own quick pick instead.
