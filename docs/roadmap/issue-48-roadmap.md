...
Revision status: REVISED after the PR #57 code review. Materially changed sections are marked **"(rev2: PR #57 review)"** with the finding number they answer — findings 1–3 of that review land in this document (§3.3 credential-injection consent, §4.2 Path B gateway-routing consent, §4.2 Path A tunnel port), and their knock-ons run through the TL;DR, §3.4, §3.5, §4.3, the cut lines, the open questions, the PR table and the risk list. Findings 4–5 land in `device-template-profiles.md`. The pre-existing "(rev)"/"REVISED READING" markers are from the earlier owner-correction pass and still stand except where a rev2 note supersedes them.
...
- **Macro editor — "Provide IPMI credentials" checkbox (rev2, new):** a checkbox bound to `provideIpmiCredentials`, placed immediately under the **"Run in"** select and shown only when Run in = *Local terminal* (`visibleWhen: { field: "runIn", value: ["localTerminal"] }` — the existing conditional-visibility mechanism, `src/ui/formTypes.ts:1-6`), so it never appears on session or browser macros where it is meaningless. Hint copy, stating exactly what is granted and where it goes:
...
**Size:** medium PR — model (`ServerConfig.ipmiAuthProfileId` + `TerminalMacro.provideIpmiCredentials`) + validation + sweep + token + flag-gated env pathway + server-form select + macro-editor checkbox + template + docs. Independent of Phase 2 (touches disjoint code except `ServerConfig`), so either order merges cleanly.
...
1. `browserTarget` (`serverMacroCommands.ts:157-193`) grows a routing step: after `resolveMacroBrowserUrl` validates the URL, if the URL's hostname equals the server's `ipmiHost` (bracketed-IPv6 normalized) **and** `ipmiGatewayServerId` is set, establish an ephemeral local tunnel — `127.0.0.1:<os-assigned> → SSH(gateway) → ipmiHost:<remotePort>` — then rewrite only the URL authority to `127.0.0.1:<port>` and open that. **`remotePort` is the URL's explicit port when it has one, otherwise the port implied by its scheme — 80 for `http:`, 443 for `https:`** (rev2: PR #57 review finding 3; the first draft's flat `?? 443` silently tunnelled `http://bmc/` to the BMC's TLS port, which either fails to connect or serves the wrong service, and does so *after* the browser has already been pointed at the local port). BMC web UIs on plain HTTP are common on older iDRAC/iLO/IPMI cards, so this is the normal case, not an edge case; an unrecognized scheme is refused with the existing URL-validation error rather than defaulted. Doing the rewrite on the **parsed resolved URL** (not in the token pass) keeps `profileTokens.ts` routing-agnostic and its authority machinery (`:585-707`) untouched.
...
**Path B — SOL by running ipmitool on the jump host (PR-C2). (rev2: PR #57 review finding 2 — routing is now a per-macro opt-in, not a server-level side effect.)**
...
0. **The routing gate.** The first draft routed *every* `localTerminal` macro of a server that has `ipmiGatewayServerId` set into the gateway session. **Withdrawn:** `runIn: "localTerminal"` is a promise about *where the command runs* — a user's `ping`, `scp`, `nmap`, `code`, or any local helper macro would, on the mere presence of an unrelated server-level field, start executing on the bastion instead. That is a silent change of execution host for commands that were never about IPMI, with real consequences (commands that don't exist on the bastion, credentials/paths that only exist locally, and audit trails that now show the bastion running a user's local tooling). A server-level field must not redefine a macro-level contract.
   ...
   **Revised: `TerminalMacro` gains a routing option**, sibling to the credentials flag of §3.3:
   ...
   - Editor surface: a **"Run on"** select next to "Run in", `visibleWhen: { field: "runIn", value: ["localTerminal"] }`, options *`This machine`* / *`The server's IPMI gateway (falls back to this machine)`*, hint: *"IPMI gateway routing runs the command in an SSH session on the gateway server configured for the target server, so tools like ipmitool can reach a BMC network that isn't reachable from here. Tokens are still resolved against the target server."*
...
### 4.3 Phase 4 tests (rev2: PR #57 review — routing-opt-in and scheme-port items added)
...
| **PR-B** | Phase 3: `ipmiAuthProfileId` + `${profile.ipmiUsername}` + **opt-in-gated** env injection (`TerminalMacro.provideIpmiCredentials` + macro-editor checkbox, rev2) + template flip (template carries the flag) + sweep + tests | M | none (independent of A; only textual overlap in `ServerConfig`) |
| **PR-C** | Phase 4 (primary, per owner's reading): `ipmiGatewayServerId` field + **`TerminalMacro.route` opt-in** + jump-host SOL routing (deliver *route=ipmiGateway* localTerminal macros into a gateway session; all others stay local, rev2) + `-a` template variant carrying the route + "Run on" editor select + docs | S–M | Ogun's answers 1–3 (light confirmation) |
...
2. **Env-var trust surface (rev2: PR #57 review)** — injecting the credential into a shell the user then owns is by design (auditability posture), and the gate is now an **explicit per-macro `provideIpmiCredentials` flag**, not the withdrawn "macro text mentions an IPMI token" heuristic (§3.3). The residual risk is that the flag be read truthily, defaulted to true anywhere, or bypassed by the prompt fallback — all three are covered by the §3.5 fixtures, and the review's own case (an imported `curl`/`ping` macro carrying `${profile.ipmiHost}` to harvest the fleet-wide BMC password) is the named regression test. What remains accepted: a flagged macro's terminal is a shell the user owns, and anything run in it can read the variable — that is the deliberate trade, now bounded by consent.
