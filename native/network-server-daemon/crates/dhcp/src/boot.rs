...
pub fn parse_vendor_option_value(raw: &str) -> Result<Vec<u8>, BootOptionError> {
    ...
    for pair in chars.chunks_exact(2) {
        ...
        let high = pair[0].to_digit(16).ok_or_else(|| BootOptionError::NonHexDigits(raw.to_owned()))?;
        let low = pair[1].to_digit(16).ok_or_else(|| BootOptionError::NonHexDigits(raw.to_owned()))?;
