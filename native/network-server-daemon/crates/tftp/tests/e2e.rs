...
fn a_traversal_attempt_never_reaches_the_filesystem() {
    let h = Harness::start(true);
    let client = h.client();
    for attempt in ["../escape.bin", "/../../etc/passwd", "a/b/../../../x"] {
...
#[cfg(windows)]
#[test]
fn a_windows_backslash_traversal_attempt_never_reaches_the_filesystem() {
    let h = Harness::start(true);
    let client = h.client();
    client.send(&encode_rrq("..\\escape.bin", "octet", &RawOptions::default()));
    match client.recv() {
        Packet::Error { code, .. } => assert_eq!(
            code,
            ErrorCode::AccessViolation.to_wire(),
            "Windows backslash traversal was not refused as an access violation"
        ),
        other => panic!("expected ERROR, got {other:?}"),
    }
    assert_eq!(h.events.started(), 0, "no session may be created for a refused path");
