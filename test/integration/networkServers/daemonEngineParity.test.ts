...
describe(`Network servers daemon — engine parity (engine=${ENGINE})`, () => {
  ...
  it("answers `getStatus` for a known id, and surfaces NOT_FOUND as a rejected promise", async () => {
    const snapshot = await host.getStatus("tftp");
    expect(snapshot.id).toBe("tftp");
    expect(snapshot.status).toBe("stopped");

    // The client maps `error.code` onto `Error.name` — a consumer branches on it.
    await expect(host.getStatus("ftp")).rejects.toMatchObject({ name: "NOT_FOUND" });
  });

  it("keeps serving after an unknown method is rejected", async () => {
    await expect(
      (host as unknown as { request(m: string): Promise<unknown> }).request("selfDestruct")
    ).rejects.toMatchObject({ name: "METHOD_NOT_FOUND" });
