...
describe("SettingsTreeProvider", () => {
  ...
  describe("root items", () => {
    ...
    // The last four are the Embedded Network Servers categories and Local
    // Servers, appended after "scripts". Each carries real settings
    // (`nexus.networkServers.verboseMode`, the `tftp.*` group, the `dhcp.*`
    // group and the `nexus.localServers.*` group), so none of them is the empty
    // row this test exists to catch.
    it("has 13 category items first, with Security & Data after SSH and no empty Inventory row (\u2298 a category kept after its last setting is removed is a settings row that opens onto nothing)", () => {
      const provider = createProvider();
      const roots = provider.getChildren();
