...
- PTY integration: `SshPty`, `SmartSerialPty`, `SerialPty` all implement `SessionPtyHandle` — `addOutputObserver(o): Disposable`, `setInputBlocked(bool)`, `writeProgrammatic(data)`, `resetTerminal()`, `markShuttingDown(reason)`. A first dropped keystroke during `setInputBlocked(true)` emits a one-shot `[Nexus] Terminal is locked…` line via the PTY's `writeEmitter`. `markShuttingDown` is called from the extension's deactivate subscription (`extension.ts:activate()` dispose hook) over every active SSH + serial session so tabs stay open with a farewell banner on reload/disable/update instead of silently hanging — it tears down the transport, sets a sticky "shutting down" flag that swallows further input, and updates the tab name to `[Disconnected]`/`[Stopped]` without firing `closeEmitter`. The handle is exposed on `ActiveSession.pty` / `ActiveSerialSession.pty` (runtime-only; not persisted).
