...
Cross-window writes: `globalState` is shared across VS Code windows, last-writer-wins, no compare-and-swap — and every `save*` persists the whole collection list, so a save from one window can overwrite another window's edit to the same collection. `VscodeConfigRepository` keeps a per-key baseline (JSON of the raw value last read/written) and, when a save would overwrite a foreign change, warns via its `onConcurrentOverwrite` hook (wired to a notification in `extension.ts`) — the write still proceeds; the loss is surfaced, not prevented (prevention isn't possible at the Memento layer). Full semantics and residual gaps in the doc comment atop `vscodeConfigRepository.ts`.
...
- **Every change PR bumps the patch version** in `package.json` (e.g. 2.7.5 → 2.7.6). Bumping is routine and carries no release consequence — it just means the work on `main` is always ready to be released. The VS Code Marketplace rejects re-publishing the same version, so a version is never re-used.
