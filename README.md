...
- **Local Shell Profiles** — Save named local terminal profiles and open one or more local shell sessions from the Connectivity Hub. Use a configured VS Code terminal profile from the profile dropdown, or choose **Custom Shell** to set an explicit shell path, one argument per line, a working directory, and an optional startup command. The VS Code profile dropdown lists only profiles that expose an explicit executable path to extensions; for WSL, create a custom Local Shell profile with `wsl.exe` as the shell path plus any required arguments. Manual macros, auto-trigger macros, and Nexus scripts work with Local Shell sessions.
