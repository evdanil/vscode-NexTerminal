...
**Start/Stop is async, like PVE itself.** A start or stop is issued to PVE, which answers immediately with a task id; Nexus polls that task — up to two minutes — and surfaces the task's own verdict, success or PVE's own message ("VM 105 already running"). The tree's **Start Node** / **Stop Node** entries are currently offered on EVE-NG lab nodes only, so Proxmox start/stop has no menu entry yet — the machinery behind it is the same, and cluster nodes cannot be started or stopped from Nexus at all.
