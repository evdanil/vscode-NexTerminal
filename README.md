...
Nodes with an HTML5/VNC console, and nodes that have no console address yet, are still imported — as servers with no address, counted in a single warning. They are deliberately not dropped: a device missing from the tree reads as *deleted at the source*, and the source's Removed-Device Policy would act on it. What such a placeholder can and can't do is below, in [Servers with no address yet](#servers-with-no-address-yet).
