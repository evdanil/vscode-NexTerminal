...
A device that carries an **out-of-band IP** in NetBox (`oob_ip`) also fills that server's **IPMI / BMC Host**, so `${profile.ipmiHost}` macros — the IPMI SOL console and BMC web console templates — work on synced servers without typing an address anywhere. Only addresses the sync itself wrote are ever updated: a value you typed by hand is never overwritten, clearing the field on one server is a per-server opt-out later syncs respect, and a device that stops reporting an out-of-band IP keeps its last known address rather than having it erased. An address NetBox reports that can't be used as a host — a URL, say — is reported in the plan's warnings instead of being stored. (One caveat if you move between versions: syncing on a build older than 2.8.97 drops the record of which addresses the sync owns. The addresses themselves survive, but those servers are treated as hand-typed from then on and stop following the source; clearing a server's IPMI / BMC Host hands it back to the sync.)
