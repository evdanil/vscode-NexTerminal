...
**Start and stop nodes from the tree.** Right-click an EVE-NG node whose state is known and choose **Start Node** (on a stopped one) or **Stop Node** (on a running one); Nexus issues the start/stop and refreshes the status a beat later once the node has begun booting. Tested against EVE-NG Community; EVE-NG Professional support is preliminary. The mechanism is provider-general: any inventory provider that can control nodes gets the same menu — for Proxmox guests, see [Sync Servers from a Proxmox Cluster](#sync-servers-from-a-proxmox-cluster). A start or stop spends the source's saved credentials, so if the extension now answering that source's provider id looks different from the one you configured the source against — its name or its fields changed — Nexus asks first, and Cancel reads no credential and sends nothing to the node. Answering **Continue** covers that one action only: the next start or stop asks again.
