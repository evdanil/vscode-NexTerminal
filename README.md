...
1. Run `Nexus: Add Inventory Source` and choose **NetBox** (Nexus now ships two inventory providers, so the first step is picking one) — or open **Settings → Inventory Sources**, which lists every configured source with inline **Sync Now**, **Edit**, **Template Rules**, and **Remove**
