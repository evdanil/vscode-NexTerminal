...
**Open the guest's console in your browser.** Right-click any Proxmox-synced guest and choose **Open Web Console**: Nexus builds the address of PVE's own noVNC console for that guest and opens it in your browser. It needs **no address of its own**, which is the point — a VM with no guest agent, or one that is simply stopped, has nothing to SSH to, and its console is the way in. The credential is the **PVE session your browser already holds**: the first open may land on PVE's login page, and after signing in the console is right there. Your API token is not involved and no new secret is stored.

The guest's current node is resolved fresh at the moment you click, so a guest migrated since the last sync opens on the node running it now. A stopped guest opens PVE's own "not running" console page — honest, and in PVE's voice. The entry is offered for **guests only**: an imported cluster node has a shell, not a guest's noVNC console, so its row carries no such entry.
...
- **Open Web Console** still works on a Proxmox-synced placeholder, for the same reason: the hypervisor's console is reached through the hypervisor, not through the guest's own address.
