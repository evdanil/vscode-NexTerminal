...
Nodes with an HTML5/VNC console, and nodes that have no console address yet, are still imported — as servers with no address. Two warning lines mention them, and both are true: EVE-NG's own line counts the nodes it returned without a telnet console, and the sync's line counts the servers it added without an address (a placeholder left over from an earlier sync appears in the first but not the second). They are deliberately not dropped: a device missing from the tree reads as *deleted at the source*, and the source's Removed-Device Policy would act on it. What such a placeholder can and can't do is below, in [Servers with no address yet](#servers-with-no-address-yet).
