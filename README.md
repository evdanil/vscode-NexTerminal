...
**Start and stop nodes from the tree.** Right-click an EVE-NG node whose state is known and choose **Start Node** (on a stopped one) or **Stop Node** (on a running one); Nexus issues the start/stop and refreshes the status a beat later once the node has begun booting. Tested against EVE-NG Community; EVE-NG Professional support is preliminary. The mechanism is provider-general: any inventory provider that can control nodes gets the same menu — for Proxmox guests, see [Sync Servers from a Proxmox Cluster](#sync-servers-from-a-proxmox-cluster).
...
A VM whose guest agent is not running — the common case, since nothing installs the agent for you — arrives as a server with no address and gains one on a later sync once the agent answers; the same is true of any guest the address crawl did not reach, because lookups are bounded at 1,000 guests or 120 seconds per sync, and a bounded sync never prunes. Both are the addressless placeholder described in [Servers with no address yet](#servers-with-no-address-yet).
