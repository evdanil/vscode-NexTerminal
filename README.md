...
- **Embedded Network Servers (TFTP + DHCP)** — Serve firmware and configs to lab hardware, and hand it addresses, without installing a separate daemon or borrowing the office DHCP server. Both services bind a NIC you pick from a live list of this machine's IPv4 addresses, and when the IANA port is denied they fall back to an unprivileged one (69 → 1069, 67 → 1067) with a warning rather than failing outright. TFTP serves read-only by default, optionally accepts uploads, sandboxes every RRQ/WRQ filename inside the configured root so a `../` in a filename cannot escape it, and lists each in-flight transfer in the sidebar with progress, speed, and the client's reverse-DNS name beside its IP — with **Cancel Transfer** on any live row and a rolling **History** of the last 50 transfers completed during the current run. DHCP runs full DORA with a dynamic pool, static MAC→IP reservations, leases that survive a restart, and the ZTP boot options a switch actually asks for: 66 and 150 (boot server), 67 (boot file), 60 (vendor-class filter), and 43 (vendor-specific TLVs) — with the boot-server address auto-linked to whatever address TFTP is bound to until you set it by hand. Reserved addresses are claimed the moment the service starts, so a device that has not booted yet cannot lose its own reservation to whichever client happens to `DISCOVER` first. **Quick Settings** is a keyboard-only pick over the handful of values a bench actually changes — interface, pool start and size, mask, gateway, lease time — and moving the pool start offers to fill in the broadcast, gateway, and DNS that follow from it; named profiles capture a whole bench setup and restore it next week. Both services run inside one isolated daemon child process, the same crash-isolation model as the serial sidecar, so a fault in a protocol engine cannot take the extension host with it and closing VS Code always releases UDP 69/67. Requires a trusted workspace.
...
| `nexus.inventory.statusPollSeconds` | `0` | How often, in seconds (`0`–`3600`), to refresh EVE-NG lab running status while the Command Center is visible. `0` disables polling — run **Refresh Lab Status** when you want it. Only EVE-NG sources report status |
