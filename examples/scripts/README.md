...
Every example assumes a session that is already open and sitting at a prompt — what **Run**, the Scripts view's ▶ quick-run and the CodeLens bind to. A script started there sees only output that arrives after it starts, so the prompt on screen is invisible to it, and most of the examples begin by pressing Enter for a fresh one (`sendLine("")`, or a `poll` that sends `\r` in 07). **Connect and Run Script…** on an SSH server is different: the script gets everything the session prints from the moment it opens, the host's first prompt included, so an Enter there leaves a spare prompt that a later wait matches too early. For that path, open the way `01-hello.js` does: wait briefly for a prompt and press Enter only if none came, which works on an open terminal too. See [Match window semantics](../../docs/scripting.md#match-window-semantics).
