...
- **A crash mid-save no longer strands a saved macro password where nothing can find it, and neither does a second VS Code window.** Every macro secret is now registered before it is written and removed only after the macro list is committed, so **Complete Reset** can sweep whatever a crash left behind, and a secret being moved to a new id is never deleted from the old one first. The register itself moved: it used to be a single list in VS Code's shared extension storage, which two windows adding a secret at the same moment could overwrite for each other — leaving one macro's password in the OS keyring where **Complete Reset** could not find it. It is now one small marker file per secret, so two windows writing at once cannot lose each other's entry. If those files cannot be written — an unwritable storage folder, a full disk, a disconnected network share — saving a **secret** macro fails with an explanation instead of storing a password Nexus could not register. Saving a list with no secret macros in it does not write any marker; the folder is still touched when a macro that *was* secret is deleted or turned into a plain one, so that its marker goes with its stored value. The old list is still maintained and still swept, so secrets registered in it by earlier versions are still found. **Complete Reset** now also asks VS Code directly for the list of secrets this extension has stored, on VS Code 1.105 and newer where that is possible, and removes every macro secret it finds — which covers the two cases no register can name: one written by an earlier version whose registration was lost to that two-window overwrite, and one re-created by a save in one window at the moment another window was deleting it. Nothing else Nexus stores is touched; server passwords, key passphrases and auth-profile secrets are left alone. On older VS Code there is no such list, so **Complete Reset** can only delete entries something still names and one of those leftovers will survive it — updating VS Code and running **Complete Reset** again clears it. Absorbing legacy `nexus.terminal.macros` settings also no longer overwrites macro changes made in another VS Code window while it was running — it retries on the next start instead.
