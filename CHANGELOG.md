...
- **A crash mid-save can no longer strand a saved macro password where nothing can find it.** Vault entries are now registered before they are written and removed only after the macro list is committed, so **Complete Reset** can always sweep them, and a secret being moved to a new id is never deleted from the old one first. Absorbing legacy `nexus.terminal.macros` settings also no longer overwrites macro changes made in another VS Code window while it was running — it retries on the next start instead.
