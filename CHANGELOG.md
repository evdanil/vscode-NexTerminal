...
- **Two macros that share an internal id no longer auto-trigger at all.** Restoring a hand-edited backup, or absorbing an old hand-written `nexus.terminal.macros` setting, could leave two macros indistinguishable to the auto-trigger — pausing one would pause the other, and resuming one could quietly arm a secret macro you never resumed. Nexus now suppresses the auto-trigger for both and flags them in the Macros view with a warning icon and an explanation, rather than guessing which macro is which. **Move Up** / **Move Down** on any macro assigns fresh ids and clears the conflict. The macro editor also now refuses to save or delete a macro whose id is shared, instead of silently overwriting the other one. Existing ids are never rewritten while loading, so a secret macro's stored password is never reassigned or deleted behind your back.
...
- **A crash mid-save no longer strands a saved macro password where nothing can find it.** Vault entries are now registered before they are written and removed only after the macro list is committed, so **Complete Reset** can sweep whatever a crash left behind, and a secret being moved to a new id is never deleted from the old one first. (One case is still open, and it needs two VS Code windows: if both add a secret macro at the same moment, one window's write to the shared macro list can overwrite the other's, and the secret belonging to the overwritten macro can be left in the OS keyring where **Complete Reset** will not find it. VS Code gives extensions no way to update shared storage atomically. Fixing it properly needs a different bookkeeping mechanism and is tracked separately.) Absorbing legacy `nexus.terminal.macros` settings also no longer overwrites macro changes made in another VS Code window while it was running — it retries on the next start instead.
- **Restoring a backup in Replace mode can no longer hand an imported macro one of your existing macros' saved passwords.** A macro's internal id in a backup file is only an id on the machine that wrote it, but Replace mode was matching those ids against local ones. With the OS keyring temporarily unavailable, an imported secret macro whose own password was missing from the backup could take over the vault entry of a local macro that happened to share its id — and then resolve to that local password, with its auto-trigger live, once the keyring came back. Replace mode now assigns every imported macro a fresh id and removes the replaced macros' stored secrets, so nothing is inherited. Merge mode is unaffected (it already skips any imported macro whose id you already have).
- **A macro secret can no longer be silently emptied by a second VS Code window.** Nexus skips rewriting a secret whose value this window has not changed, so a stale window cannot put an old password back over a new one. But if the other window deleted that macro — or ran **Complete Reset** — in the meantime, skipping left the macro on disk marked as secret with no stored value behind it, which reads back as an empty secret with no warning. Nexus now checks that the stored value is still there before skipping, and restores it if it has gone. A newer value written by the other window is still left alone.
